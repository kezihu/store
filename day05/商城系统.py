...
def shopping():
    ...
    def receipt():
        ...
        for name, price in enumerate(mycart, 1):
            print('\t\t\t\t\t', name, price)
        if use == 1 and a > 10:
            print('您本次共购买了', j, '个商品，并且使用了一张联想电脑优惠券，共消费', consume, '元，您还剩余', money, '元！')
        elif use == 1 and a <= 10:
            print('您本次共购买了', j, '个商品，并且使用了一张老干妈优惠券，共消费', consume, '元，您还剩余', money, '元！')
        else:
                    ...
                    if chose >= 7:  # 输入的商品不存在
                        ...
                        if use == 0:
                            if money < (shop[chose][1]) * 0.1:
                                ...
                                mycart.append(shop1[chose])  # 将选择的商品添加到购物车中
                                consume = consume + (shop[chose][1]) * 0.1
                                money = money - (shop[chose][1]) * 0.1
                                print('-'.center(60, '-'))
                                print('购买成功，已使用抽到的联想电脑优惠券，', shop[chose][0], '已加入到您的购物车里!剩余余额：￥', money)
                                use = use + 1
                                j = j + 1
                        else:
                        ...
                        if use == 0:
                            if money < (shop[chose][1]) * 0.7:
                                ...
                                print('-'.center(60, '-'))
                                print('购买成功，已使用抽到的老干妈优惠券，', shop[chose][0], '已加入到您的购物车里!剩余余额：￥', money)
                                use = use + 1
                                j = j + 1
                        else:
                            if money < shop[chose][1]:
