...
my_pass = 'yhvirjronelbbgdd'  # 发件人邮箱授权码
user = '87281094@qq.com'  # 收件人邮箱账号

msg = MIMEMultipart()  # 创建一个邮件
msg['From'] = formataddr(("刘锦克", sender))  # 括号里对应发件人昵称、发件人邮箱账号
msg['To'] = formataddr(("", user))  # 括号里对应收件人昵称、收件人邮箱账号
...
att = MIMEText(open('bank.html', 'rb').read(), 'base64', 'utf-8')  # 构造附件，两个个参数：第一个为附件路径，第二个附件格式
att["Content-Type"] = 'application/octet-stream'
att["Content-Disposition"] = "attachment; filename='bank.html'"  # filename为文件名字
msg.attach(att)
try:
